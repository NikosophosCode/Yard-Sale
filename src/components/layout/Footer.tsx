...
/**
 * Footer de la aplicación con links útiles y redes sociales
 *
 * @example
 * ```tsx
 * <Footer />
...
export function Footer({ className }: FooterProps) {
  ...
  const socialLinks = [
    {
      name: 'Twitter',
      href: 'https://.linkedin.com/in/nikosophoscode',
      icon: (
    ...
    >
      <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-5">
          ...
          {/* Shop Links */}
          <div>
            <h3 className="mb-4 text-sm font-semibold tracking-wider text-neutral-900 uppercase dark:text-neutral-100">
              Shop
            </h3>
            <ul className="space-y-3">
              {footerLinks.shop.map((link, index) => (
                <li key={`shop-${index}`}>
                  <a
                    href={link.href}
                    className="hover:text-brand-600 dark:hover:text-brand-400 text-sm text-neutral-600 transition-colors dark:text-neutral-400"
                  >
                    {link.label}
                  </a>
                </li>
        ...
        <div className="mt-12 border-t border-neutral-200 pt-8 dark:border-neutral-800">
          <p className="text-center text-sm text-neutral-600 dark:text-neutral-400">
            © {currentYear} Great Sale. All rights reserved. Built by nikosophoscode with ❤️.
          </p>
        </div>
      </div>
    </footer>
